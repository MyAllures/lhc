...
class Wsl extends Common{
    public function index(){
        ...
        $res_zx=Db::name('bl')->field('distinct(class2)')->where(['class1'=>'尾数连'])->select();
        foreach($res_zx as $kz=>$vz){
            ...
            if ($row['lx']==4){
                $rz1=Db::name('tan')->field('sum(sum_m) as sum_m,sum(sum_m*guan1_zc/10) as sum_mzc ')->where(['kithe'=>$kithe,'lx'=>0,'guan1'=>$row['kauser'],'class2'=>$vz['class2']])->select();
            }
            if ($_SESSION['lx']==2){
        ...
        foreach ($result as $v){
            ...
            for ($j=0; $j<$ssc; $j=$j+1){
                if($t_sql==""){
                    ...
                    $t_sql.=" or class3='".$xxm[$j]."'";
                }
            }
            $rw=Db::query("Select * from ka_bl  where   class1='".$v['class1']."' and class2='".$v['class2']."' and (".$t_sql.")");
            $Rsbl=$rw[0];
