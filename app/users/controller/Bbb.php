...
class Bbb extends Common{
    ...
    public function server_tm(){
        ...
        $z7_sum=0;
        $z7_ds=0;
        if ($row['lx']==4){ $vvvv=['guan'=>$row['kauser']] ; }
...
//        }
        ...
        for($i=0;$i<$ii;$i++)
        {
            $sum_sx[$i]=$z_suma-$z_ds-$z_pz*((1-getQuotaField(27,'yg')/100))-($sum_ma[$i]-$sum_pz1[$i])*$sum_bl[$i];
//if ($i==0){
...
//$sum_sx[$i]=($z_suma+$z_sumb+$z_ds)+$sum_xx[$i]+$sum_sx1[$i]-$z_pz+$z7_ds;}
        ...
        for($i=0;$i<$ii;$i++)
        {
            ...
            if(($sum_sx[$i]+$ztm_tm)>=0 || $sum_bl[$i]==0 ){
                $ffxx=0;}else{
                $ffxx=(-$sum_sx[$i]-$ztm_tm)/($sum_bl[$i]+getQuotaField(27,'yg')/100-1);
                /*
                if ($i==0){
